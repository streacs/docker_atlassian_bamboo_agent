...
function build_container {
  case $APPLICATION_BRANCH in
    ...
    feature*)
      ...
      docker build --no-cache -t ${DOCKER_REPOSITORY}/${APPLICATION_NAME}:feature --build-arg APPLICATION_SERVER=${APPLICATION_SERVER} --build-arg APPLICATION_RELEASE=${APPLICATION_RELEASE} .
    ;;
    *)
      echo "No match found"
    ;;
...
function test_container {
  case $APPLICATION_BRANCH in
    master)
      docker run -ti --rm --env-file files/environment.list ${DOCKER_REPOSITORY}/${APPLICATION_NAME}:master rake spec /home/bamboo/spec
    ;;
    develop)
      docker run -ti --rm --env-file files/environment.list ${DOCKER_REPOSITORY}/${APPLICATION_NAME}:develop rake spec /home/bamboo/spec
    ;;
    release*)
      APPLICATION_RELEASE="$(git symbolic-ref --short HEAD | grep -o -E "(\d{1,2}\.){2,3}\d")"
      docker run -ti --rm --env-file files/environment.list ${DOCKER_REPOSITORY}/${APPLICATION_NAME}:${APPLICATION_RELEASE} rake spec /home/bamboo/spec
    ;;
    feature*)
      docker run -ti --rm --env-file files/environment.list ${DOCKER_REPOSITORY}/${APPLICATION_NAME}:feature rake spec /home/bamboo/spec
    ;;
    *)
      echo "No match found"
    ;;
