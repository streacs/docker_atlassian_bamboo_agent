...
function build_container {
  case $APPLICATION_BRANCH in
    ...
    feature*)
      ...
      docker build --no-cache -t ${DOCKER_REPOSITORY}/${APPLICATION_NAME}:feature-${GIT_HASH} --build-arg APPLICATION_SERVER=${APPLICATION_SERVER} --build-arg APPLICATION_RELEASE=${APPLICATION_RELEASE} .
    ;;
    *)
      echo "No match found"
    ;;
...
function test_container {
  case $APPLICATION_BRANCH in
    master)
      docker run -t --rm --env-file files/environment.list ${DOCKER_REPOSITORY}/${APPLICATION_NAME}:master rake spec /home/bamboo/spec
    ;;
    develop)
    ...
    feature*)
      GIT_HASH="$(git rev-parse --short HEAD)"
      docker run -t --rm --env-file files/environment.list ${DOCKER_REPOSITORY}/${APPLICATION_NAME}:feature-${GIT_HASH} rake spec /home/bamboo/spec
    ;;
    *)
      echo "No match found"
    ;;
